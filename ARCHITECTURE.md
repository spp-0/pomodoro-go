...
```
        ...
        │   3) q := fetchQuote(cur, ...)       │ 在线 quote
        │   4) popupQueue <- popupJob{...}     │ 投到 UI 队列
        └──────────────────┬───────────────────┘
        ...
        │       ui.ShowPopup(job.e, job.q,    │ ← webview2 在这里
        │                  job.opts)           │
        │   }                                   │
        └──────────────────┬───────────────────┘
...
| **emitter 内部 fetchQuote** | 同步 `http.Get` | 最多 1500ms | 不发弹窗，只取 quote |
...
4. **`Scheduler.UpdateConfig` 会清空当天 `lastT` 标记**。改配置后 17:18 已触发的会被允许再触发；如果要"永久"标记，不要用 UpdateConfig 走 reload，用其它接口。
...
main()
  ...
  ├─ startUIDispatcher(logger)        // LockOSThread 消费 popupQueue
...
未实现但设计上预留了扩展点（详见 [DEVELOPING.md](./DEVELOPING.md)）：
- snooze 按钮（弹窗里"5/10/15 分钟后再提醒"）
- 多显示器支持（用所在屏幕的 work area）
- 多文案轮换（每个时间点配自己的 title/message）
- 开机自启开关（写入 `shell:startup` 或任务计划程序）
- 统计（每天完成多少番茄钟）
- 多账号 / 多配置文件
