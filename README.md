...
- **指定时间点列表**：每天 10:30 / 14:30 / 17:30 等时间各弹一次（每项可单独配标题/内容）
- **两种可同时启用**，互不影响
- **弹窗精美**：渐变背景 + 卡片 + 倒计时 + ESC/Enter 关闭；弹窗内显示**当前城市天气**
- **稍后提醒（Snooze）**：每个弹窗可一键「5/10/15 分钟后再提醒」
- **随机诗词/名言**：弹窗里附一句中文诗词或名人名言（在线 + 离线兜底）
- **托盘三态图标 + 实时倒计时**：🔴 专注 / 🟡 休息 / ⚪ 暂停，悬停显示剩余时间与今日番茄数
- **统计**：自动记录每日完成的番茄钟（设置页查看近 7 天）
- **勿扰/会议模式**：托盘一键开启，弹窗暂不弹出但番茄钟继续计时
- **快捷预设**：设置页一键套用 25/5、50/10、90/20
- **多语言界面**：简体中文 / 繁體中文 / English / 日本語 / 한국어（设置页切换后托盘菜单即时刷新；仅界面文案本地化，提醒内容与诗词保持原样）
- **弹窗高可靠**：每个弹窗独立线程互不拖累；Go 侧自动关闭 + 看门狗双兜底，绝不卡死；WebView2 不可用时回退系统消息框，提醒不丢失
...
- **设置…**：打开图形化设置窗口（番茄钟、时间点、提醒音、天气、弹窗位置、诗词 API、开机自启、统计）
- **暂停调度**（点选切换）：所有提醒暂停；再次点击恢复
- **勿扰模式（会议）**（点选切换）：弹窗暂不弹出，但番茄钟继续计时
- **跳过当前休息** / **延长休息 5 分钟**：仅休息阶段可用
- **声音**（点选切换）：弹窗时是否播放提醒音
- **开机自启**（点选切换）：登录时自动启动
...
### v1.1.0（2026-07-27）
- **修复**：scheduler 两处自死锁（`UpdateConfig` 锁内 emit、`setStateLocked` 锁内同步回调），导致点「立即弹窗」程序整体卡死、弹窗不出（详见 DEVELOPING.md §5.6）
