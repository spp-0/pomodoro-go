...
- **弹窗天气**：设置页开启天气并填城市（支持中文/拼音/英文），弹窗内先显示「正在获取天气…」占位，约 1~3 秒后显示天气图标 + 城市 + 温度 + 文字；常见中国城市已内置坐标，避免海外 geocoding 对部分中文城市返回 not found；网络异常时占位变为「天气获取失败」（弹窗本身正常）；失败原因会写进 `pomodoro.log`（`[weather] fetch failed for city=...: ...`），可据此判断是超时还是网络不可达；改城市后重新弹窗显示新城市天气。
