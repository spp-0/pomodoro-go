...
- **弹窗天气**：设置页开启天气并填城市，弹窗内显示天气图标 + 城市 + 温度 + 文字（天气是**异步**获取的，弹窗先秒出、天气随后注入，约 1~3 秒到位）；网络异常时静默失败（弹窗正常，无天气块），失败原因会写进 `pomodoro.log`（`[weather] fetch failed: ...`），可据此判断是超时还是网络不可达；改城市后重新弹窗显示新城市天气。
